...
# ── Trend: Mobile SA1-4 by district, from the nightly Drive backups ─────
# Separate from the rest of this module - the report above is a fixed
# snapshot from the uploaded workbook, but this pulls live historical data
# from the same ~01:15 backup system Flood NAN Monitoring and the P0
# snapshot cards use, matched against Summary NAN's own site->district
# mapping (from the uploaded "main" sheet) rather than the broader Flood
# NAN site master, so district labels stay consistent with the rest of
# this tab.
...
def _classify_sa_mobile_by_district(rows, site_district_by_ciname):
    """rows: raw ticket row dicts (live or from a downloaded backup).
    Returns (district_counts, total) for SA Mobile tickets matched to a
    Summary NAN site via CINAME."""
    from pending_ticket import ALLOWED_SEVERITIES, _exclusive_bookmark_label
    ...
    for r in rows:
        ...
        district = site_district_by_ciname.get(ciname)
        if district is None:
            continue
        district_counts[district] = district_counts.get(district, 0) + 1
        total += 1
    return district_counts, total


def _get_trend_day(gs_client, drive_service, day, site_district_by_ciname):
    """Cached per-day lookup - always sourced from the nightly ~01:15
    backup (including "today", whose backup has typically already landed
    by the time anyone is looking at this during the day), so all 5 points
    are consistently from the same snapshot system. Cached forever once
    computed, same reasoning as the other backup-driven trends in this
    app: a written backup file never changes."""
    from pending_trend import find_nightly_file, download_xlsx_as_rows

    date_str = day.strftime("%Y-%m-%d")
    with _trend_cache_lock:
        cached = _trend_day_cache.get(date_str)
    ...
    if file_info is None:
        ...
        result = _classify_sa_mobile_by_district(rows, site_district_by_ciname)

    with _trend_cache_lock:
        _trend_day_cache[date_str] = result
    return result


def build_summary_nan_trend(gs_client, drive_service, days=5):
    from datetime import timedelta
    from pending_trend import bangkok_now

    sites_raw, _tickets_raw = _get_raw()
    site_district_by_ciname = {
        s["site_id"].strip().upper(): (s.get("district") or "").strip() or UNSPECIFIED_ROOT_CAUSE
        for s in sites_raw if s.get("site_id")
    }
    ...
    for day in all_days:
        ...
        district_counts, total = _get_trend_day(gs_client, drive_service, day, site_district_by_ciname)
        district_by_date[date_str] = district_counts
        total_series.append(total)

    all_districts = sorted({d for date_str in dates for d in district_by_date[date_str]})
    district_series = {
        d: [district_by_date[date_str].get(d, 0) for date_str in dates]
